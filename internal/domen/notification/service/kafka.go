...
func (s *Service) KafkaGetMessage(message *sarama.ConsumerMessage) {
	...
	case model.TopicOrderNotification:
		var whMessage model.NotificationMessage
		err := json.Unmarshal(message.Value, &whMessage)
		if err != nil {
			s.logger.Error("Step 1- Error unmarshalling message from OrderService", "err", err)
			return
		}
		err = s.domenNotificationSend(ctx, whMessage)
		if err != nil {
			s.logger.Error("Step 2- Error lunching worker task to notificate", "err", err)
			return
		}
		s.logger.Info("Adding worker to notificate", "Order ID", whMessage.OrderID)
	default:
